...
export default defineEventHandler(async (event) => {
  ...
  const cascadeDelete = db.transaction(() => {
    ...
    // 5. Delete section_terms explicitly for this term, then orphaned sections
    db.prepare('DELETE FROM section_terms WHERE term = ?').run(termStr)
    db.prepare('DELETE FROM sections WHERE id_section NOT IN (SELECT id_section FROM section_terms)').run()
