...
const makeupClassStats = computed(() => {
  if (!makeupClasses.value) return { total: 0, confirmed: 0, completed: 0 }

  return {
    total: makeupClasses.value.length,
    confirmed: makeupClasses.value.filter(m => m.status === 'confirmed').length,
    completed: makeupClasses.value.filter(m => m.status === 'completed').length
  }
})

const confirmedCount = computed(() =>
  makeupClasses.value.filter(m => m.status === 'confirmed').length
)
...
const successRate = computed(() => {
  ...
  return Math.round((makeupClassStats.value.confirmed / makeupClasses.value.length) * 100)
})
