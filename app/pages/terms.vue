<template>
  <div class="min-h-screen bg-white text-slate-900 pb-20">
      ...
      <div class="container mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div class="flex gap-4">
          <div class="bg-slate-50 px-6 py-3 rounded-2xl border border-slate-200 backdrop-blur-sm">
            <p class="text-lg font-black uppercase tracking-widest text-slate-500 mb-1">จำนวนเทอมทั้งหมด</p>
            <p class="text-2xl font-black text-slate-900">{{ terms?.length || 0 }} <span
                class="text-lg font-normal text-slate-500">รายการ</span></p>
          </div>
          <div class="bg-slate-50 px-6 py-3 rounded-2xl border border-slate-200 backdrop-blur-sm">
            <p class="text-lg font-black uppercase tracking-widest text-slate-500 mb-1">ปีการศึกษาล่าสุด</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
          <div class="flex items-center justify-between mb-6">
            ...
            <div class="text-lg text-slate-500 italic">เรียงตามปีการศึกษาล่าสสุด</div>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div v-for="term in sortedTerms" :key="term.id_term"
              ...
              <div class="flex flex-col sm:flex-row justify-between items-center relative z-10 gap-6">
                <div class="flex items-center gap-6 w-full">
                  ...
                  <div>
                    <div class="text-2xl font-black text-slate-900 flex items-center gap-2">
                      ...
                      <span v-if="isCurrentTerm(term)"
                        class="text-[10px] bg-green-500/20 text-green-600 px-2 py-0.5 rounded-full border border-green-500/30 whitespace-nowrap">ปัจจุบัน</span>
                    </div>
                    <div class="text-slate-600 text-md flex items-center gap-2 mt-2 font-medium">
                      ...
                      <UIcon name="i-heroicons-arrow-right-circle" class="text-slate-400" /> {{
                ...
                <div
                  ...
                  <UButton icon="i-heroicons-trash" color="error" variant="soft" label="ลบรายการ" size="md"
                    class="rounded-xl flex-1 sm:flex-none border border-red-500/20 cursor-pointer"
                    :disabled="deletingTermId === term.id_term" :loading="deletingTermId === term.id_term"
    ...
    <!-- Modal ยืนยันการลบ -->
    <UModal v-model:open="confirmDeleteTerm"
      ...
      <template #content>
        <div class="flex flex-col max-h-[85vh]">
          <div class="p-8 overflow-y-auto custom-scrollbar flex-1">
            ...
            <p class="text-slate-600 text-center mb-8">การดำเนินการนี้จะลบข้อมูลตารางเรียนและข้อมูลที่เกี่ยวข้องทั้งหม
              ดในเทอมนี้อย่างถาวร ไม่สามารถย้อนคืนได้</p>
          ...
          <div class="p-6 border-t border-slate-200 bg-white/95 backdrop-blur-sm sticky bottom-0 z-10">
            <div class="flex flex-col sm:flex-row gap-3">
              <UButton label="ยกเลิก" color="neutral" variant="outline" size="xl" block
                class="rounded-2xl border-slate-200 py-4 flex-1" @click="confirmDeleteTerm = false" />
