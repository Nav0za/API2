    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div v-else-if="!sections || sections.length === 0"
        ...
        <div
          ...
          <UIcon name="i-heroicons-user-group" class="text-3xl text-slate-400" />
        </div>
        <h3 class="text-xl font-bold text-slate-900 mb-2">ยังไม่มีกลุ่มเรียน</h3>
        <p class="text-slate-500 mb-6">เริ่มสร้างกลุ่มเรียนแรกเพื่อจัดการตารางสอนได้เลย</p>
          ...
          <div class="p-8 overflow-y-auto custom-scrollbar flex-1">
            <div class="flex items-center gap-4 mb-8">
              <div
                ...
                <UIcon name="i-heroicons-plus-circle" class="text-2xl text-blue-600" />
              </div>
              <div>
                <h3 class="text-2xl font-bold text-slate-900">เพิ่มกลุ่มเรียน</h3>
                <p class="text-slate-500 text-sm">สร้างกลุ่มนักศึกษาใหม่สำหรับจัดการตารางเรียน</p>
              </div>
            </div>
          ...
          <div class="p-8 overflow-y-auto custom-scrollbar flex-1">
            <div class="flex items-center gap-4 mb-8">
              <div
                ...
                <UIcon name="i-heroicons-pencil-square" class="text-2xl text-amber-600" />
              </div>
              <div>
                <h3 class="text-2xl font-bold text-slate-900">แก้ไขข้อมูลกลุ่มเรียน</h3>
                <p class="text-slate-500 text-sm">การเปลี่ยนชื่อกลุ่มจะไม่ส่งผลต่อตารางเรียนเดิม</p>
              </div>
            </div>
            ...
            <form @submit.prevent="handleEditSection" class="space-y-6">
              ...
              <UFormField label="รายละเอียด" :ui="{ label: 'text-slate-900 font-bold mb-2' }">
                <UTextarea v-model="editingSection.description" placeholder="รายละเอียด..." size="xl" class="rounded-xl"
                  :rows="3" :ui="{ base: 'bg-white border-slate-200 text-slate-900 rounded-2xl' }" />
              </UFormField>
            </form>
          </div>
          <div class="p-6 border-t border-slate-200 bg-white/95 backdrop-blur-sm sticky bottom-0 z-10">
      ...
      <template #content>
        <div class="flex flex-col max-h-[85vh]">
          <div class="p-8 overflow-y-auto custom-scrollbar flex-1">
            ...
            <h3 class="text-2xl font-bold text-slate-900 text-center mb-2">ยืนยันการลบกลุ่มเรียน</h3>
            <p class="text-slate-500 text-center mb-8">คุณแน่ใจหรือไม่ที่จะลบกลุ่มเรียน <span
                class="text-slate-900 font-bold">{{ deletingSection?.section_name }}</span>?
          ...
          <div class="p-6 border-t border-slate-200 bg-white/95 backdrop-blur-sm sticky bottom-0 z-10">
            <div class="flex flex-col sm:flex-row gap-3">
              <UButton label="ยกเลิก" color="neutral" variant="outline" size="xl" block
                class="rounded-2xl border-slate-200 py-4 flex-1 font-bold" @click="openDeleteModal = false" />
